% SNR_N (hold n_t=16,K_2=10) K_1=[1 10 50 100] 
clc;
clear;
N = 50:50:500;
%s = rng(2);
...
%% gain
figure(1);
plot(N,10*log10(real(result_RA_aver(:,1))),'-ob','LineWidth',0.5);
hold on;
plot(N,10*log10(real(result_MO_aver(:,1))),'-^k','LineWidth',0.5);
plot(N,10*log10(real(result_RA_aver(:,2))),'--ob','LineWidth',0.5);
plot(N,10*log10(real(result_MO_aver(:,2))),'--^k','LineWidth',0.5);
plot(N,10*log10(real(result_RA_aver(:,3))),':ob','LineWidth',0.5);
plot(N,10*log10(real(result_MO_aver(:,3))),':^k','LineWidth',0.5);
plot(N,10*log10(real(result_RA_aver(:,4))),'-.ob','LineWidth',0.5);
plot(N,10*log10(real(result_MO_aver(:,4))),'-.^k','LineWidth',0.5);

%plot(N,10*log(real(result_sdr_average)),'-b','LineWidth',2);
legend('RA,$K_1=0$','MO,$K_1=0$','RA,$K_1=1$','MO,$K_1=1$','RA,$K_1=10$','MO,$K_1=10$','RA,$K_1=50$','MO,$K_1=50$','Interpreter', 'latex','NumColumns',2);
xlabel('$N,n_t=16$','Interpreter', 'latex','FontSize',14);
